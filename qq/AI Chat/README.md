...
- `config.yaml`：`admin_uins`（高权限组）、`rate_limit`（每日限额）、`context`（上下文条数，含 `group_context_limit` 群聊上下文条数与 `reply_context_enabled` 引用回复开关）、`chat_history.db_path`（可选，历史库路径）。首次使用复制 `configExample.yaml` 为 `config.yaml`（`config.yaml` 已被 gitignore，不入库）。
- `prompts.yaml`：无输入引导语
